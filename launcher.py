...
def run_browser_as_user(command):
    ...
    if not user:
        ...
        print(f"[M] Запуск браузера от текущего пользователя: {' '.join(command)}")
    ...
    try:
        ...
        if pid == 0:
            ...
            print(f"[M] Запуск браузера от пользователя {user}: {' '.join(command)}")
            try:
                subprocess.Popen(command)
            except Exception as e:
                print(f"[M] Ошибка запуска браузера: {e}")
            finally:
                os._exit(0)
        else:
        ...
        print(f"[M] Не удалось переключиться на пользователя {user}: {e}")
        print(f"[M] Запуск браузера от root (может не работать с GUI): {' '.join(command)}")
        subprocess.Popen(command)
...
def open_browser_kiosk():
    ...
    # Проверяем, какой браузер доступен
    if shutil.which("chromium-browser"):
        cmd = ["chromium-browser", "--kiosk", url]
        if is_root:
            cmd.insert(1, "--no-sandbox")
        print(f"[M] Используется chromium-browser")
        run_browser_as_user(cmd)
    elif shutil.which("chromium"):
        cmd = ["chromium", "--kiosk", url]
        if is_root:
            cmd.insert(1, "--no-sandbox")
        print(f"[M] Используется chromium")
        run_browser_as_user(cmd)
    elif shutil.which("firefox"):
        cmd = ["firefox", "--kiosk", url]
        print(f"[M] Используется firefox")
        run_browser_as_user(cmd)
    else:
        webbrowser.open(url)
        print("[M] Не удалось найти браузер с поддержкой kiosk. Открыто в обычном режиме.")
    ...
    while time.time() - start_time < timeout:
        try:
            with socket.create_connection((host, port), timeout=1):
                print(f"[M] Сервер доступен на {host}:{port}")
                return True
        except (socket.timeout, ConnectionRefusedError):
            time.sleep(0.5)
    print(f"[M] Сервер не стал доступен за {timeout} секунд")
    return False
...
def open_browser_when_ready():
    ...
    if wait_for_server(timeout=15):
        ...
        print("[M] Не удалось дождаться сервера, браузер не запущен.")
