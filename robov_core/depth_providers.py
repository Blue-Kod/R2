...
class StereoSGBMDepthProvider(DepthProvider):
    ...
    def compute(self, left_gray: np.ndarray, right_gray: np.ndarray) -> DepthResult:
        left_eq = self._clahe.apply(left_gray)
        right_eq = self._clahe.apply(right_gray)

        disp = self.left_matcher.compute(left_eq, right_eq)
        disp[disp < 0] = 0

        if self._wls_enabled and self.right_matcher and self.wls_filter:
            disp_r = self.right_matcher.compute(right_eq, left_eq)
            disp = self.wls_filter.filter(disp, left_eq, disparity_map_right=disp_r)
            disp[disp < 0] = 0
